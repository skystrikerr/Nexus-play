...
import type { Game, GamingSession } from "@shared/schema";

interface GamingCalendarProps {
  games: Game[];
}

export default function GamingCalendar({ games }: GamingCalendarProps) {
  ...
  const { data: sessions = [] } = useQuery<GamingSession[]>({
    queryKey: ["/api/sessions", { startDate, endDate }],
  });

  const { data: todaySessions = [] } = useQuery<GamingSession[]>({
    queryKey: ["/api/sessions", { date: todayDateString }],
  });
